...
// homemade score command
Window_MenuCommand.prototype.addScoreCommand = function() {
    if (this.needsCommand('score')) {
        var enabled = this.isContinueEnabled();
        this.addCommand(TextManager.options, 'score', enabled);
    }
};
...
//-----------------------------------------------------------------------------
// Window_ScorefileList
//
// The window for rendering the score screens.

function Window_ScorefileList() {
    this.initialize.apply(this, arguments);
}

Window_ScorefileList.prototype = Object.create(Window_Selectable.prototype);
Window_ScorefileList.prototype.constructor = Window_ScorefileList;

Window_ScorefileList.prototype.initialize = function(x, y, width, height) {
    Window_Selectable.prototype.initialize.call(this, x, y, width, height);
    this.activate();
    this._mode = null;
};

Window_ScorefileList.prototype.setMode = function(mode) {
    this._mode = mode;
};

Window_ScorefileList.prototype.maxItems = function() {
    return DataManager.maxSavefiles();
};

Window_ScorefileList.prototype.maxVisibleItems = function() {
    return 5;
};

Window_ScorefileList.prototype.itemHeight = function() {
    var innerHeight = this.height - this.padding * 2;
    return Math.floor(innerHeight / this.maxVisibleItems());
};

Window_ScorefileList.prototype.drawItem = function(index) {
    var id = index + 1;
    var valid = DataManager.isThisGameFile(id);
    var info = DataManager.loadScorefileInfo(id);
    var rect = this.itemRectForText(index);
    this.resetTextColor();
    if (this._mode === 'score') {
...
Window_ScorefileList.prototype.drawContents = function(info, rect, valid) {
    var bottom = rect.y + rect.height;
    if (rect.width >= 420) {
    ...
    if (y2 >= lineHeight) {
        this.drawScore(info, rect.x, y2, rect.width);
    }
};

Window_ScorefileList.prototype.drawGameTitle = function(info, x, y, width) {
    if (info.title) {
        this.drawText(info.name, x, y, width);
    }
};

Window_ScorefileList.prototype.drawPartyCharacters = function(info, x, y) {
    if (info.characters) {
...
Window_ScorefileList.prototype.drawFileId = function(id, x, y) {
    this.drawText("Top " + id, x, y, 180);
};

Window_ScorefileList.prototype.drawScore = function(info, x, y, width) {
    if (info.playtime) {
        this.drawText("Score : "+info.gold+" - Temps : "+info.playtime, x, y, width, 'right');
    }
};

Window_ScorefileList.prototype.playOkSound = function() {
};
//-----------------------------------------------------------------------------
// Window_OneScore (homemade)
//
// The window that shows the score details

function Window_OneScore() {
    this.initialize.apply(this, arguments);
}

Window_OneScore.prototype = Object.create(Window_MenuStatus.prototype);
Window_OneScore.prototype.constructor = Window_OneScore;

Window_OneScore.prototype.initialize = function() {
    Window_MenuStatus.prototype.initialize.call(this, 0, 0);
    this.hide();
};

Window_OneScore.prototype.processOk = function() {
    if (!this.cursorAll()) {
        $gameParty.setTargetActor($gameParty.members()[this.index()]);
    }
    this.callOkHandler();
...
Window_OneScore.prototype.selectLast = function() {
    this.select($gameParty.targetActor().index() || 0);
};

Window_OneScore.prototype.selectForItem = function(item) {
    var actor = $gameParty.menuActor();
    var action = new Game_Action(actor);
    action.setItemObject(item);
    this.setCursorFixed(false);
...
Window_SavefileList.prototype.drawPlaytime = function(info, x, y, width) {
    if (info.playtime) {
        ...
        this.drawText("Points : "+info.gold+" - Temps : "+info.playtime, x, y, width, 'right');
    }
};
...
//-----------------------------------------------------------------------------
// Window_ScoreCommand (homemade)
//
// The Window for checking the high scores on title screen. 

function Window_ScoreCommand() {
    this.initialize.apply(this, arguments);
}

Window_ScoreCommand.prototype = Object.create(Window_HorzCommand.prototype);
Window_ScoreCommand.prototype.constructor = Window_ScoreCommand;

Window_ScoreCommand.prototype.initialize = function(width, scoreList) {
    this._windowWidth = width;
    this._scoreList = scoreList;
    Window_HorzCommand.prototype.initialize.call(this)
}

Window_ScoreCommand.prototype.windowWidth = function() {
    return this._windowWidth;
}

Window_ScoreCommand.prototype.maxCols = function() {
    return 3;
};

Window_ScoreCommand.prototype.makeScoreList = function() {
    // return score here 
...
Window_TitleCommand.prototype.makeCommandList = function() {
    ...
    this.addCommand(TextManager.options,   'score', enabled); // homemade
};
