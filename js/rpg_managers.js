...
DataManager.loadScoreInfo = function() {
    var json;
    try {
    ...
    if (json) {
        var scoreInfo = JSON.parse(json);

        var finalInfo = [null];// the table that will render
        var noZero = scoreInfo; // we make a table that we will sort
        delete noZero[0]; // get rid of index zero cause it's empty
        
        noZero = sortObj(noZero, 'gold'); // sort by gold
        noZero.reverse(); // from big to tiny

        // push all this into the final
        for (var i=0; i<=noZero.length;i++) {
            finalInfo.splice(i,0,noZero[i]);
        }
        var scoreInfo = finalInfo; // bypass
        var noZero, finalInfo; // free mem
...
DataManager.loadScorefileInfo = function(savefileId) {
    var scoreInfo = this.loadScoreInfo();
    return (scoreInfo && scoreInfo[savefileId]) ? scoreInfo[savefileId] : null;
};
...
DataManager.makeSavefileInfo = function() {
    ...
    info.name       = $gameParty.members()[0].name();
